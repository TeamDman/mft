...
pub fn resolve_paths_simple<'a>(file_names: &'a [FileNameRef<'a>], per_entry: &[Vec<usize>]) -> eyre::Result<Vec<Option<PathBuf>>> {
    ...
    while changed {
        ...
        for entry_id in 0..entry_count {
            ...
            let fname_indices = &per_entry[entry_id];
            if fname_indices.is_empty() { continue; }
            let candidates: Vec<&FileNameRef> = fname_indices.iter().filter_map(|&idx| file_names.get(idx)).collect();
